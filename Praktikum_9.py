...
class AplikasiPrediksi(tk.Tk):
    def __init__(self):
        ...
        self.btn_update = tk.Button(
            ...
            command=self.update_nilai_action, bg="#FFC107", fg="black", font=("Arial", 9, "bold")
        )
        self.btn_update.pack(side="left", padx=5)

        self.btn_delete = tk.Button(
            btn_frame, text="Delete", width=12, 
            command=self.delete_nilai_action, bg="#F44336", fg="white", font=("Arial", 9, "bold")
        )
        ...
        # Bind klik pada Treeview untuk mengisi form
    ...
    def on_tree_select(self, event):
        """Mengisi form berdasarkan baris yang dipilih di Treeview"""
    ...
    def update_nilai_action(self):
        ...
        try:
            update_nilai(nim, nama, biologi, fisika, inggris, prediksi)
    ...
    def delete_nilai_action(self):
        nim = self.ent_nim.get().strip()
        if not nim:
            msg.showwarning("Peringatan", "Pilih data dari tabel atau isi NIM untuk dihapus.")
            return
        
        if msg.askyesno("Konfirmasi", f"Apakah Anda yakin ingin menghapus data dengan NIM {nim}?"):
            try:
                delete_nilai(nim)
